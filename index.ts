/**
 * dispensable-ask - a user-controlled question tool for Pi.
 *
 * The interactive UI is adapted from pi-ask-user by Enzo Lucchesi (MIT).
 */
...
import {
   ...
   parseTimeoutSeconds,
   saveConfig,
   type DispensableAskConfig,
} from "./config";

import { createRequire } from "node:module";
const _require = createRequire(import.meta.url);
const DISPENSABLE_ASK_VERSION: string = (_require("./package.json") as { version: string }).version;
const TOOL_NAME = "dispensable_ask";
 ...
 */
async function runDialogBeforeDeadline<T>(
   deadline: number,
   onTimeout: () => void,
...
export default async function dispensableAsk(pi: ExtensionAPI) {
   let config: DispensableAskConfig = await loadConfig();
   let enabled = false;
   let activeAsk = false;

   const statusText = () => `ask:${enabled ? "on" : "off"} · ${formatTimeout(config.timeoutSeconds)}`;

   function refreshStatus(ctx: ExtensionContext): void {
      ctx.ui.setStatus(STATUS_KEY, statusText());
   }

   function applyExposure(nextEnabled: boolean): void {
      enabled = nextEnabled;
      const activeTools = pi.getActiveTools().filter((name) => name !== TOOL_NAME);
      pi.setActiveTools(nextEnabled ? [...activeTools, TOOL_NAME] : activeTools);
   }

   function setExposureFromUser(nextEnabled: boolean, ctx: ExtensionContext): void {
      if (activeAsk) {
         ctx.ui.notify("dispensable_ask is waiting for an answer; its state was not changed", "warning");
         return;
      }

      applyExposure(nextEnabled);
      refreshStatus(ctx);
      ctx.ui.notify(
         `dispensable_ask ${nextEnabled ? "enabled" : "disabled"} for this session`,
         nextEnabled ? "info" : "warning",
      );
   }

   function disableAfterTimeout(ctx: ExtensionContext): void {
      applyExposure(false);
      refreshStatus(ctx);
      ctx.ui.notify(
         `dispensable_ask timed out after ${formatTimeout(config.timeoutSeconds)} and is disabled for this session`,
         "warning",
      );
   }

   pi.registerShortcut(config.shortcut as any, {
      description: "Toggle dispensable_ask exposure",
      handler: (ctx) => setExposureFromUser(!enabled, ctx),
   });

   pi.registerCommand("dispensable-ask", {
      description: "Show, enable, disable, or configure dispensable_ask",
      handler: async (args, ctx) => {
         ...
         if (action === "status") {
            ctx.ui.notify(
               `dispensable_ask is ${enabled ? "enabled" : "disabled"}; timeout ${formatTimeout(config.timeoutSeconds)}; toggle ${config.shortcut}; config ${getConfigPath()}`,
               "info",
            );
            return;
         }
         ...
         if (action === "on" || action === "enable") {
            setExposureFromUser(true, ctx);
            return;
         }

         if (action === "off" || action === "disable") {
            setExposureFromUser(false, ctx);
            return;
         }

         if (action === "toggle") {
            setExposureFromUser(!enabled, ctx);
            return;
         }
         ...
         if (action === "timeout" && parts.length === 2) {
            ...
            const nextConfig = { ...config, timeoutSeconds };
            try {
               await saveConfig(nextConfig);
               config = nextConfig;
               refreshStatus(ctx);
               ctx.ui.notify(`Global dispensable_ask timeout set to ${formatTimeout(timeoutSeconds)}`, "info");
            } catch (error) {
               ctx.ui.notify(`Could not save dispensable_ask config: ${String(error)}`, "error");
            }
            return;
         }
   ...
   // Removing a tool cannot retract a schema already sent to an in-flight
   // model request. Block that stale call so disabling is still immediate.
   pi.on("tool_call", (event) => {
      if (event.toolName === TOOL_NAME && !enabled) {
         return {
            block: true,
            reason: "dispensable_ask is disabled for this session; continue without asking the user",
         };
      }
   });
   ...
   pi.registerTool({
      ...
      promptGuidelines: [
         "Before calling dispensable_ask, gather available context and pass a short summary via the context field.",
         "Prefer dispensable_ask over repeatedly reconsidering the same user-dependent decision.",
         "Ask exactly one focused question per dispensable_ask call.",
         "Do not ask for confirmation of a decision the user already made.",
         "If dispensable_ask times out or is disabled, continue with your best judgment instead of retrying.",
      ],
      // Block other tool calls in the same assistant turn until the user answers,
      // so the model can't batch dispensable_ask with bash/edit/write and let those run
      ...
      async execute(_toolCallId, params, signal, onUpdate, ctx) {
         if (!enabled) {
            return {
               content: [{ type: "text", text: "dispensable_ask is disabled; continue without asking the user" }],
               isError: true,
               ...
               content: [
                  {
                     ...
                     text:
                        ...
                        + `Call dispensable_ask again with corrected options.`,
                  },
               ],
               isError: true,
         ...
         if (options.length === 0) {
            ...
            if (!response) {
               if (timedOut) {
                  disableAfterTimeout(ctx);
                  return {
                     content: [{
                        type: "text",
                        text: `No answer was received within ${formatTimeout(config.timeoutSeconds)}. dispensable_ask is now disabled for this session. Continue using your best judgment and do not retry unless the user manually re-enables it.`,
                     }],
                     details: {
                        question,
            ...
            if (
               ...
               removeOverlayInputListener = ctx.ui.onTerminalInput((data) => {
                  ...
                  if (nextHidden && !hasAnnouncedHide) {
                     ...
                     ctx.ui.notify?.(`dispensable_ask hidden — press ${overlayToggle.spec} to reopen`, "info");
                  }
                  return { consume: true };
               });
            }
      ...
      renderCall(args, theme) {
         ...
         let text = theme.fg("toolTitle", theme.bold("dispensable_ask "));
